...
public class NoticeController extends HttpServlet {
	...
	protected void goEventList(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//1. 사용자가 클릭한 페이지를 가져온다 but, 처음 이벤트페이지 왔을시 1로 초기화
		int page = 0;
		if(request.getParameter("page") == "" || request.getParameter("page") == null) {
			page = 1;
		}else {
		...
		int firstPage = 0, lastPage = 0;
		ArrayList<Integer> pageList = null;
		int allEventCnt = 0 , allPageCnt = 0;
		String select = "", searchText = "";
		
		
		//3. 전체 페이지 개수를 구한다. ex. 게시글이 36개이면 총 4개의 페이지를 보여줄 수 있다.
		if(request.getParameter("searchText") == null) {
			allEventCnt = noticeService.getEventCnt();
			
		}else {
			select = request.getParameter("select");
			searchText = request.getParameter("searchText");
			allEventCnt = noticeService.getEventCnt(select, "%"+searchText+"%");
		}
		
		if(allEventCnt%10==0) {
			allPageCnt = allEventCnt/10; 
		}else {
			allPageCnt = (allEventCnt/10)+1;
		}
		...
		if(request.getParameter("searchText") == null) {
			eventList = noticeService.getEventList(start, end);
		}else {
			eventList = noticeService.getEventList(start, end, select, "%"+searchText+"%"); 
		}
		...
		request.setAttribute("firstPage", firstPage);
		request.setAttribute("pageList", pageList);
		request.setAttribute("noticeList", eventList);	
	...
	protected void goEventDetail(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		Notice notice = noticeService.selectEventDetail(noticeNo);
		request.setAttribute("notice", notice); //여기서 제목,번호,작성자 ~~ 다저장된 notice자체를 보냈자나요 그래서
